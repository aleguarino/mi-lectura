 ...
 ********************************************************/
package com.guarino.milectura.ui.stats;
...
// Avanza una semana completa para obtener lastDay
        for (int i = 0; i < 7; i++) {
            cal.add(Calendar.DATE, 1);
        }
        cal.add(Calendar.DATE, -1);  // Ajusta para obtener el último día de la semana correcta
        long lastDay = cal.getTimeInMillis();

        List<ReadingDate> readings = ReadingRepository.getInstance().getPagesWeekly(firstDay, lastDay);

        LinkedHashMap<Long, Integer> pagesReadedWeekly = new LinkedHashMap<>();
        ...
        if (readings.size() > 0) {
            ...
            cal.add(Calendar.DAY_OF_MONTH, -7); // Vuelve al inicio de la semana

            for (int i = 0; i < 7; i++) {
                long day = cal.getTimeInMillis();
                int pages = pagesReadedWeekly.getOrDefault(day, 0);
